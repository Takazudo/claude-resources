# Complete Tauri Sidecar App

A Tauri v2 desktop app that wraps a local dev server. On launch it spawns `pnpm dev:stable` as a sidecar process, shows a bundled loading page, polls the server via IPC, and navigates to the docs once the build is ready.
...
The bundled loading page shown while the dev server builds. Uses Tauri IPC to poll the `check_ready` command, then navigates via `navigate_to_docs` once the server responds with 200.
...
<body>
  ...
  <script>
    // Use Tauri IPC to check server readiness, then navigate via Rust
    (function poll() {
      if (window.__TAURI__) {
        window.__TAURI__.core.invoke('check_ready').then(function(ready) {
          if (ready) {
            window.__TAURI__.core.invoke('navigate_to_docs');
          } else {
...
- **Login shell for sidecar**: `/bin/zsh -l -c` ensures the user's PATH is available when the app is launched from Finder (where PATH is minimal)
- **Process group cleanup**: `process_group(0)` creates a new process group so `kill(-pid, SIGTERM)` terminates the entire tree, not just the shell
- **`check_ready` via raw TCP**: avoids pulling in an HTTP client crate; issues a minimal `GET /___ready HTTP/1.0` and checks for a 200 status line
- **IPC-driven navigation**: the frontend polls `check_ready` via `window.__TAURI__.core.invoke()`, then triggers `navigate_to_docs` to let Rust handle the actual URL navigation -- avoids cross-origin issues with direct `location.href` changes
- **Zoom via `document.body.style.zoom`**: simple JS eval from Rust, state tracked in `AppState.zoom`
- **WindowEvent::Destroyed exits the app**: overrides macOS default behavior where closing the window only hides it
