...
- If the files appear **still useful** (referenced in the current conversation): **move them** to the log directory: `LOGDIR=$(node ~/.claude/scripts/get-logdir.js) && mkdir -p "$LOGDIR" && mv <file> "$LOGDIR/"`
