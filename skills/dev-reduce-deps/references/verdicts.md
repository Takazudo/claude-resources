...
- **A KEEP justified by "an upstream framework retains it" goes stale when that upstream drops the
  dep — re-check retention-based KEEPs after first-party bumps.**
...
| 2026-09-15 | `gray-matter` | js | KEEP | zzmod | Terminal KEEP in root + product-md-sync. A 17-line yaml-based wrapper proved 0 byte-level diffs vs gray-matter across 3554 blobs/91 refs, but skeptic vote was 1 uphold/1 inconclusive/1 refute: wrapper+14 import swaps+~16 call-site renames breaks the 40-line abandon rule, product-md-sync cannot cleanly import a root wrapper, and 6 tests use matter.read/matter.stringify. doc's own declaration still removed (R1) since it was only held alive by dead code (content-files.ts). |
...
| 2026-09-15 | `shiki` | js | REMOVE | zzmod | Removed from doc/ (-26 packages) and sub-packages/styleguide (part of R8, with @shikijs/transformers, -25 combined). Consumer html-preview/highlighted-code.tsx no longer exists; zfb highlights via the syntect WASM highlighter since zudo-doc 4.3. All 3 skeptics upheld. |
| 2026-09-15 | `minisearch` | js | REMOVE | zzmod | Removed from doc/ (-1 package, R2): zudo-doc 5.24.2 no longer imports it, an undeclared-peer KEEP that expired with an upstream refactor. Root and styleguide keep their own direct-import declarations (R9 in styleguide was declaration-hygiene-only, 0 package payoff, since root already declares it). |
| 2026-09-15 | `remark-cjk-friendly` | js | REMOVE | zzmod | Removed from doc/ (-11, R3) and styleguide (-5, R10): zfb renders markdown/CJK in Rust; cjkFriendly is a settings flag, not the npm plugin. No string reference in the zfb binary. |
| 2026-09-15 | `@takazudo/zfb-adapter-cloudflare` | js | REMOVE | zzmod | Removed from doc/ (R6) and sub-packages/styleguide (R13), 0 package payoff (declaration hygiene only, root keeps a real declaration): neither doc/zfb.config.ts nor styleguide's zfb.config.ts sets an adapter, and zfb only pulls an adapter when one is configured. Styleguide's wrangler.toml also confirms it isn't used. |
| 2026-09-15 | `hono` | js | REMOVE | zzmod | Removed from sub-packages/zmdpreview (R15), 0 package payoff (zero-package hygiene): zero references, carried over from the zfb 0.1.0-next.19 migration; zfb-runtime already imports hono from its own dependency tree. |
| 2026-09-15 | `vite` | js | DECIDE | zzmod | Root lockfile carries vite 8.1.3 + 8.2.0 duplicated. A scratch pnpm dedupe collapses to 971 packages (-37) but is NOT behavior-neutral: it moves every vite 8.1.3 edge to 8.2.0 and drops rolldown 1.1.3 + 15 platform bindings + other transitives. Deliberately not run/committed with round 260915's removals; needs its own change verified like a version bump. |
| 2026-09-20 | `mermaid` | js | REMOVE | zudo-sg | zudo-doc 5.25 loads pinned Mermaid from esm.sh; the unused root Mermaid 12 declaration removed 109 lockfile package records |
| 2026-09-20 | `minisearch` | js | REMOVE | zudo-sg | the current search widget uses built-in scoring with no MiniSearch import; removing the root declaration removed one package record |
| 2026-09-20 | `remark-cjk-friendly` | js | REMOVE | zudo-sg | zfb 2.19 owns CJK-friendly Markdown handling natively; the obsolete root declaration removed seven package records |
| 2026-09-20 | `minisearch` | js | REMOVE | zudo-ez-host | zudo-doc 5.26.2 has no MiniSearch runtime or peer import; custom scorer and 39-entry EN/JA search index verified; removing doc declaration removed one lockfile package record. |
| 2026-09-20 | `pagefind` | js | REMOVE | zudo-ez-host | zudo-doc 5.26.2 emits search-index.json without Pagefind scripts/assets; EN/JA 39-entry build and unavailable-index fallback verified; removing doc declaration removed eight lockfile package records. |
