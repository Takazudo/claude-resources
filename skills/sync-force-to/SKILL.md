...
description: >-
  ...
  re-pushing. Before overwriting, creates a backup PR so the old state can be restored via GitHub's
  "Restore branch" button. Use when 'sync-to' failed due to conflicts, or when user says 'sync force
  to', 'force sync to', 'force push to branch', or wants to overwrite a target branch with their
  current branch.
