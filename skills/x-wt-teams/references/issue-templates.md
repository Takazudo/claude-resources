...
Generate a structured report. Two destinations: log directory (reachable later via `/cclogs`) and the linked GitHub issue (for human visibility).
...
- **Same reviewer flag** as this session (i.e. `-co` when it was used). Forward whatever was used.
