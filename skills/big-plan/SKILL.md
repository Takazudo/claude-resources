...
- **`-toco` / `--to-codex` flag**: Codex hand-off — the **plan on Claude Code → implement on Codex CLI** direction, a sibling of `-br`'s plan-locally → implement-on-web. In place of Step 11's in-session Skill-tool auto-invoke, delegate to **`/tocodex`**, which opens a **new tmux window** in the current session running `codex` at the repo root, stages the implementation command there, submits it, and focuses that window; this session ends at the hand-off. It **replaces** the local implementation chain rather than adding to it, so nothing is ever implemented twice. Like `-a` / `-m` it **triggers** the Step 11 chain — a plain `/big-plan -toco` hands off instead of stopping at the summary — and it forwards whichever of `-a` / `-m` / `-nf` / `-nori` / `-lo` were passed into the Codex command. Every Step 11 pause condition still applies: a design-decision plan, a foreign parent branch, or unresolved Step 9 verification stops the hand-off exactly as it stops the Skill-tool invoke. **Terminal-only** (web has neither tmux nor a local Codex, so it is inert there) and overridden by `-po`, which ends the chain at planning — though `/tocodex` can be typed later in that session, or a fresh one, to hand the finished plan over then. See the Step 11 sub-section **`-toco` / `--to-codex` — hand off to Codex in a new tmux window**.
...
one-command hand-off — lives in **[`references/issue-sweep.md`](references/issue-sweep.md)**. Read
...
**It fires at the end, here, in place of the Skill-tool invocation above** — planning has to finish first, because the epic and sub-issues are what Codex is being pointed at. (In `/x-as-pr` and `/x-wt-teams` the same flag fires at the *start* and does not route through `/tocodex` — see their shared spec, [`codex-handoff.md`](../x-wt-teams/references/codex-handoff.md).) Everything before this point is unchanged: print the summary and decisions table, then evaluate the three pause conditions in the same order. **A pause stops the hand-off exactly as it stops the in-session invoke** — do not hand off a plan that needs a human decision first.
...
**Do not forward reviewer flags or effort levels** — the Codex session picks its own, the same rule the in-session hand-off follows. `/tocodex` submits by default; pass `-ns` through only if the user typed it.
