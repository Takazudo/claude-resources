...
## Kill Stale Port Before Start

When the app crashes or is force-quit, the old dev server may survive and hold the port. Kill it before starting a new one:
...
## Health Check: Accept Any HTTP Response

When polling the server URL, don't require HTTP 200. Frameworks with a non-root `baseUrl` (e.g., Docusaurus `baseUrl: "/pj/app/doc/"`) return 404 for `/`. Any HTTP response means the server is up:
...
function isServerReady(url) {
  return new Promise((resolve) => {
    ...
    const req = http.request(
      ...
      // CORRECT: any response means server is alive
