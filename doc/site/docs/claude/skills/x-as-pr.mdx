...
When a GitHub issue is linked (either passed as argument or created via `--make-issue`), **add a TODO checklist** to the issue body to track workflow progress. This prevents losing track of steps during long implementations.

### Adding the TODO

When creating an issue (`--make-issue`) or linking an existing one, ensure the issue body contains this checklist (use `gh issue edit` to append if needed):
...
## Branch Rule: Stay on the Working Branch

**IMPORTANT: Do NOT checkout `main` or any other branch when done.** After the workflow completes (PR creation, implementation, local review, push, CI watch, PR revision), remain on the working branch (`&lt;BRANCH_NAME&gt;`). The user expects to still be on the branch they were working on. Never switch away from it at the end.
